...
import type { SmileTable, SmileColumn } from "./types";
...
type DrizzleColumnBuilder =
  ...
  | SQLiteCustomColumnBuilder<string>;
 ...
 */
export function toDrizzle(table: SmileTable): ReturnType<typeof sqliteTable> {
  ...
  return sqliteTable(table.name, columns, (t) => {
    ...
    table.indexes.forEach(indexDef => {
      ...
      if (indexColumns.length > 0) {
        const idx = index(indexDef.name).on(indexColumns[0], ...indexColumns.slice(1));
        if (indexDef.unique) {
