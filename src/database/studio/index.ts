...
import { fromZod, getValidatedTable } from "../zod";
...
export const spawnDrizzleStudio = async (nuxt: Nuxt) => {
	...
	nuxt.options.runtimeConfig.public.smile.database = defu(
		nuxt.options.runtimeConfig.public.smile.database,
		{
			studioPort,
		},
	);
	...
	nuxt.hook("build:before", async () => {
		...
		const schema = await generateSchemaFile(
			templateDir,
			Object.values(smileRuntimeConfig.experiments),
		);
	...
	nuxt.hook("listen", async () => {
		studioProcess = spawn(
			"npx",
			["drizzle-kit", "studio", "--port", `${studioPort}`],
			{
				cwd: root,
				stdio: "inherit",
			},
...
async function generateDrizzleConfig(
	templateDir: string,
	runtimeConfig: SmileRuntimeConfig,
): string {
	const templateData = {
	...
	const file = await readFile(
		join(templateDir, `drizzle.config.ts.handlebars`),
		{ encoding: "utf-8" },
	);
...
async function generateSchemaFile(
	...
	const tables = [
		...
		...experiments.flatMap((experiment) => {
			const experimentTable = getValidatedTable(
	...
	const file = await readFile(join(templateDir, `schema.ts.handlebars`), {
		encoding: "utf-8",
	});
	const template = Handlebars.compile(file);
	return template(templateData);
